...
- **tvOS : réaffecter un `@FocusState` à la valeur qu'il a déjà ne fait rien.** Quand un bouton est remplacé par un autre au même emplacement (ex. « Demander » → « Fermer »), écrire `focus = .primary` est un no-op — SwiftUI ne voit aucun changement, et il remet lui-même le binding à `nil` quand l'ancien bouton disparaît, *après* la ligne. Résultat : le focus pointe sur une vue disparue, la télécommande ne répond plus. Il faut `focus = nil` puis réaffecter au tick suivant (`Task { @MainActor in ... }`).
