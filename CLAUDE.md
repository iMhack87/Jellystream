...
- **tvOS : un `.onMoveCommand` avale les flèches de TOUT son sous-arbre**, qu'il les traite ou non — un `guard … else { return }` arrête le seek, pas l'interception. Un panneau ou une carte à plusieurs boutons placé SOUS ce modificateur ne peut donc jamais déplacer son focus : le bouton suivant est inatteignable à la télécommande, et Entrée valide toujours le premier. C'est ce qui a rendu « Not now » inaccessible pendant deux versions de la carte de fin d'épisode, sans que rien ne le signale — le bon bouton s'affichait, avec son anneau de focus. Parade : le handler descend sur la **couche de lecture** (vidéo + pilule de skip) et les surcouches deviennent ses sœurs, pas ses filles. Vérifié à la télécommande : les flèches déplacent le focus dans la carte ET continuent de seeker pendant la lecture.
- **Trois actions tiennent sur un téléviseur, pas sur un téléphone tenu à la verticale.** La carte de fin d'épisode peut en porter trois (lire / demander la saison / plus tard) : `FlowRow` côté Compose, `ViewThatFits` côté SwiftUI. En `Row`/`HStack` nu, le troisième bouton sort de la carte et s'imprime à une lettre par ligne — invisible en revue, évident au premier lancement sur un téléphone.
...
- **La démo publique n'a AUCUN sous-titre** (0 item sur 17), une seule série, et il n'existe **aucune instance Jellyseerr publique**. D'où deux bancs locaux dans `tools/` :
  ...
  ```
  Les deux se complètent : le banc Jellyfin sert une série dont **seule la saison 1 existe**, avec le TMDb id 95396 que le banc Jellyseerr connaît comme Severance (S1 dispo, S2 demandable). Ses 3 épisodes de 40 s couvrent les trois formes de la carte de fin d'épisode en trois minutes : fin de l'**E1** → « Up next » seul avec décompte ; fin de l'**E2** → carte combinée (épisode suivant + demande de saison 2, sans décompte) ; fin de l'**E3** → offre de saison seule. Finir le 3ᵉ épisode déclenche donc la proposition de demander la saison 2, de bout en bout. Le banc Jellyseerr fait **bouger la progression** avec l'horloge ; `POST /api/v1/bench/reset` remet les fixtures à zéro entre deux runs.
