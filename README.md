...
**Browsing.** A home screen with **Continue Watching** and **Next Up**, **collections**, **genres**, movie and episode detail pages, **series browsing** (seasons and episodes), and **search**. Tapping a genre or a face in the **cast row** opens that catalogue. Detail pages show the **audience score and the Rotten Tomatoes tomatometer**, colored fresh or rotten. Watched episodes are ticked, and unwatched previews stay blurred so a thumbnail never spoils the next episode. You choose **which libraries reach the home screen** — the music and photo ones a video player has no use for start switched off, one tap from coming back.

**Playing.** Anything playable **Direct Plays the original file** — mpv+FFmpeg on Apple platforms (the all-formats engine), Media3/ExoPlayer on Android with FFmpeg audio decoders, and **libmpv as a fallback** when Media3 refuses a file — with pause/seek controls, **audio and subtitle track selection**, **Skip Intro** driven by the server's media segments, **the next episode offered and auto-played**, cross-device resume, and progress reporting back to the server. An **Info overlay** shows codec, HDR, fps and Direct Play vs transcode; **chapters** jump into the file; the TV **matches its refresh rate** to the film. **AirPlay** (iPhone/iPad) and **Chromecast** send an HLS transcode to the other screen — Direct Play stays on the glass in your hand. A per-profile **Always transcode** switch is there for the rare file a device cannot decode.
